...
export default function FolderForm() {
  ...
  return <Page className="max-w-4xl"><Header title={editing ? 'Edit folder' : 'Create folder'} subtitle={!editing ? 'Image, Video, and Movements are added automatically.' : undefined} back /><div className="glass-strong mt-6 rounded-[26px] p-5 sm:p-8">{folder.isError ? <ErrorState message={apiMessage(folder.error)} retry={folder.refetch} /> : <form className="grid gap-6" onSubmit={handleSubmit((values) => save.mutate(values))}><Field label="Name" placeholder="e.g. Dress" autoFocus={!editing} error={errors.name?.message} {...register('name')} /><Field label="Description (optional)" placeholder="What belongs in this folder?" multiline className="min-h-32" error={errors.description?.message} {...register('description')} /><div className="flex justify-end"><Button type="submit" title={editing ? 'Save changes' : 'Create folder'} loading={save.isPending} /></div></form>}</div></Page>;
}
